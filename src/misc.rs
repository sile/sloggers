...
use types::{Severity, SourceLocation, ProcessID, TimeZone};
...
pub fn build_with_options(drain: Fuse<Async>,
                      level: Severity,
                      kvfilterparameters: &Option<KVFilterParameters>,
                      source_location: SourceLocation,
                      process_id: ProcessID) -> Logger {
	if let Some(ref p) = kvfilterparameters {
		...
		match (source_location, process_id) {
			...
			(SourceLocation::ModuleAndLine, ProcessID(false)) =>
				Logger::root(drain.fuse(), o!(
                       "module" => FnValue(module_and_line),
                    )),
			(SourceLocation::None, ProcessID(true)) =>
				Logger::root(drain.fuse(), o!(
                       "pid" => FnValue(getpid),
                    )),
			(SourceLocation::ModuleAndLine, ProcessID(true)) =>
				Logger::root(drain.fuse(), o!(
                       "module" => FnValue(module_and_line),
                       "pid" => FnValue(getpid),
                    )),
